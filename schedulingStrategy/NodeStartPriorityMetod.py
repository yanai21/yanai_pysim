    ...
    # Idleノードに割り当てられない時
        ...
        if NUM_NODES_Preemption != 0:
            ...
            preemptionJobs, preemptionNodes, reservedNodes = PreemptionAlgorithm(
                urgentJob, Nodes, now, event, preemptionJobs, preemptionNodes, result, reservedNodes
            )
        # NodeStart
        if NUM_NODES_NodeStart != 0:
            startNodes, reservedNodes = NodeStart(
                NUM_NODES_NodeStart, NUM_SLEEP_NODES, NUM_NODES, urgentJob, now, Nodes, event, startNodes, reservedNodes
            )
        # 緊急ジョブの割り当て時刻の決定
